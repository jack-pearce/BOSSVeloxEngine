...
/// Return the Date filter expression as per data format.
    ...
    std::vector<RowVectorPtr> getVeloxPlanBuilder(
      ...
      for (auto itExpr = veloxExprList.begin(); itExpr != veloxExprList.end(); ++itExpr) {
        ...
        auto results = runQuery(plan.planNode(), taskCursor);
#ifdef DebugInfo
        printResults(results);
        std::cout << std::endl;
#endif
        if (itExpr == veloxExprList.end() - 1) {
          std::cout << "Velox Finished." << std::endl;
          return results;
        }
        auto lastResult = results.at(0);
        outputLayout = lastResult.get()->type()->asRow().names();
        auto lastResultNode = std::make_shared<core::ValuesNode>(
                planNodeIdGenerator->next(), results, false);
        tableMapPlan.push_back(std::move(lastResultNode));
      }
    }
