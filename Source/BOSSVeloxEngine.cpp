...
class StatelessOperator : public Engine {
    public:
        ...
        template<typename TypedFunctor>
        StatelessOperator &operator=(TypedFunctor f) {
...
void bossExprToVelox(Expression const &expression) {
  std::visit(
    ...
    boss::Expression Engine::evaluate(boss::Expression&& e) {
      ...
      operators["Select"_] = [this](boss::ComplexExpression &&inputExpr) -> Expression {
...
static auto &enginePtr(bool initialise = true) {
  ...
  if (!engine && initialise) {
    ...
    boss::engines::velox::benchmark.initialize();
    boss::engines::velox::queryBuilder = std::make_shared<BossQueryBuilder>(toFileFormat("parquet"));
    auto data_path = "../../velox_test_ZUpx2q";
    boss::engines::velox::queryBuilder->initialize(data_path);
  }
  return engine;
}
