...
//                          return boss::Span<Element>(std::move(input));
    ...
    ExpressionSpanArgument veloxtoSpan(const VectorPtr &vec) {
      if ((vec->typeKind() == TypeKind::BIGINT) ||
        ...
        auto ptr = vec->values()->as<int64_t>();
        auto size = vec->size();
        // this destructor is used only if the data is owned (i.e. if the ptr is not yet in the reference counter)
        spanReferenceCounter.add((void *) ptr, [arr = std::move(vec)]() {});
        return boss::Span<const int64_t>(ptr, size,
                                         [ptr](auto && /*unused*/) { spanReferenceCounter.remove((void *) ptr); });
      } else if ((vec->typeKind() == TypeKind::DOUBLE) ||
                 (vec->typeKind() == TypeKind::REAL)) {
        auto ptr = vec->values()->as<double_t>();
        auto size = vec->size();
        // this destructor is used only if the data is owned (i.e. if the ptr is not yet in the reference counter)
        spanReferenceCounter.add((void *) ptr, [arr = std::move(vec)]() {});
        return boss::Span<const double_t>(ptr, size,
                                          [ptr](auto && /*unused*/) { spanReferenceCounter.remove((void *) ptr); });
      } else {
...
// "Table"_("Column"_("Id"_, "List"_(1, 2, 3), "Column"_("Value"_ "List"_(0.1, 10.0, 5.2)))
    ...
    boss::Expression Engine::evaluate(boss::Expression &&e) {
      ...
      auto tmpResults = getVeloxPlanBuilder(veloxExprList, columnAliaseList, taskCursor);
      auto results = tmpResults[0];
